...
#%% functions
def load_data():
    filepath = "./qachqouch_ANN_dataset_dummy.txt"
    ...
    T = np.asarray(data['Tmean_950'])
    res = fit_sin(tt, T)
    data['Tsin'] = res["fitfunc"](tt)
    
    #do not use all the data
    data.drop(columns=['precip_thiessen','SR_precipitation','SolarRad_kWm2_Avg_1750',
                       'SolarRad_MJm2_Tot_1750','wind speed avg_950','pet'],inplace=True)
...
def simulate_testset(densesize, seqlength, batchsize, filters,Tmax_950,Tmin_950,Tmean_950,Tmax_1850,Tmin_1850,Tmean_1850,Tsin,Rad_950,rH_950):
    ...
    if not Tmax_950:
        data.drop(columns=['Tmax_950'],inplace=True)
    if not Tmin_950:
        data.drop(columns=['Tmin_950'],inplace=True)
    if not Tmean_950:
        data.drop(columns=['Tmean_950'],inplace=True)
    if not Tsin:
        data.drop(columns=['Tsin'],inplace=True)
    if not Tmax_1850:
        data.drop(columns=['Tmax_1850'],inplace=True)
    if not Tmin_1850:
        data.drop(columns=['Tmin_1850'],inplace=True)
    if not Tmean_1850:
        data.drop(columns=['Tmean_1850'],inplace=True)
    if not rH_950:
        data.drop(columns=['Relative Humidity avg_950'],inplace=True)
    if not Rad_950:
        data.drop(columns=['total solar radiation_950'],inplace=True)
...
def bayesOpt_function(densesize, seqlength, batchsize, filters,Tmax_950,Tmin_950,Tmean_950,Tmax_1850,Tmin_1850,Tmean_1850,Tsin,Rad_950,rH_950):
    ...
    Tmax_950 = int(round(Tmax_950))
    Tmin_950 = int(round(Tmin_950))
    Tmean_950 = int(round(Tmean_950))
    Tmax_1850 = int(round(Tmax_1850))
    Tmin_1850 = int(round(Tmin_1850))
    Tmean_1850 = int(round(Tmean_1850))
    Tsin = int(round(Tsin))
    rH_950 = int(round(rH_950))
    Rad_950 = int(round(Rad_950))
    ...
    repeat = True # some initializations do no work properly, repeat until acceptable performance
    iniplus = 0
    while repeat:
        for ini in range(inimax):
...
with tf.device("/gpu:0"):
    ...
    optsteps1 = 40 # random initial steps
    optsteps2 = 80 # least no of steps
    ...
    GLOBAL_SETTINGS = {
        ...
        'learning_rate': 1e-3,
        'verbose': 2,
        'stopset_start': pd.to_datetime('01102018', format='%d%m%Y'),
        'optset_start': pd.to_datetime('01022019', format='%d%m%Y'),
        'test_start': pd.to_datetime('01102019', format='%d%m%Y'),
        'test_end': pd.to_datetime('22012020', format='%d%m%Y')
    }
    ...
    Tmax_950 = int(round(optimizer.max.get("params").get("Tmax_950")))
    Tmean_950 = int(round(optimizer.max.get("params").get("Tmean_950")))
    Tmin_950 = int(round(optimizer.max.get("params").get("Tmin_950")))
    Tmax_1850 = int(round(optimizer.max.get("params").get("Tmax_1850")))
    Tmean_1850 = int(round(optimizer.max.get("params").get("Tmean_1850")))
    Tmin_1850 = int(round(optimizer.max.get("params").get("Tmin_1850")))
    Tsin = int(round(optimizer.max.get("params").get("Tsin")))
    rH_950 = int(round(optimizer.max.get("params").get("rH_950")))
    Rad_950 = int(round(optimizer.max.get("params").get("Rad_950")))


    #run test set simulations

    scores, TestData, sim1, obs1, inimax, testresults_members_all, sim1_uncertainty = simulate_testset(densesize, seqlength, batchsize, filters,Tmax_950,Tmin_950,Tmean_950,Tmax_1850,Tmin_1850,Tmean_1850,Tsin,Rad_950,rH_950)
...
pyplot.title("Qachqouch Spring", size=15)
...
pyplot.savefig('Plot_Qachqouch.png', dpi=500)
pyplot.show()
...
printdf.to_csv('./results_Qachqouch.txt',sep=';', float_format = '%.6f')