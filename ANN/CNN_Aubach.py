...
#%% functions
def load_data():
    filepath = "/aubach_ANN_dataset_dummy.txt"
    ...
    T = np.asarray(data['T_WalmHorn'])
    res = fit_sin(tt, T)
    data['Tsin'] = res["fitfunc"](tt)
    
    #do not use snow routine inputs and PET
    data.drop(columns=['WLSR_WalmHorn','WLSR_Diedamskopf','WLSR_Oberstdorf','PET_WalmHorn','PET_Oberstdorf'],inplace=True)
...
def simulate_testset(densesize, seqlength, batchsize, filters,T_WalmHorn,T_Diedamskopf,T_Oberstdorf,Tsin):
    ...
    if not T_WalmHorn:
        data.drop(columns=['T_WalmHorn'],inplace=True)
    if not T_Diedamskopf:
        data.drop(columns=['T_Diedamskopf'],inplace=True)
    if not T_Oberstdorf:
        data.drop(columns=['T_Oberstdorf'],inplace=True)
    if not Tsin:
    ...
    for ini in range(inimax):
        ...
        if not os.path.isdir('./' + model_name):
            ...
            print("model loading successful: ini"+str(ini))
...
def bayesOpt_function(densesize, seqlength, batchsize, filters,T_WalmHorn,T_Diedamskopf,T_Oberstdorf,Tsin):
    #translate into usabable parameters
    densesize = 2**int(densesize)
    seqlength = 6*int(seqlength)
    batchsize = 2**int(batchsize)
    filters = 2**int(filters)
    
    T_WalmHorn = int(round(T_WalmHorn))
    T_Diedamskopf = int(round(T_Diedamskopf))
    T_Oberstdorf = int(round(T_Oberstdorf))
    Tsin = int(round(Tsin))
...
with tf.device("/cpu:0"): #or gpu
    ...
    pbounds = {'seqlength': (2,90*24/4), # 90 days, in 6h steps
               'densesize': (4,9),
               'batchsize': (4,9),
               'filters': (4,9),
               'T_WalmHorn': (0,1),
    ...
    # optimize while improvement during last n steps
    ...
    T_WalmHorn = int(round(optimizer.max.get("params").get("T_WalmHorn")))
    T_Oberstdorf = int(round(optimizer.max.get("params").get("T_Oberstdorf")))
    T_Diedamskopf = int(round(optimizer.max.get("params").get("T_Diedamskopf")))
    Tsin = int(round(optimizer.max.get("params").get("Tsin")))


    #run test set simulations

    scores, TestData, sim1, obs1, inimax, testresults_members_all, sim1_uncertainty = simulate_testset(densesize, seqlength, batchsize, filters,T_WalmHorn,T_Diedamskopf,T_Oberstdorf,Tsin)
...
pyplot.title("Aubach Spring", size=15)
...
pyplot.savefig('Plot_Aubach.png', dpi=500)
pyplot.show()
...
printdf.to_csv('./results_Aubach.txt',sep=';', float_format = '%.6f')