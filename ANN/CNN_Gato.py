...
#%% functions
def load_data():
    filepath = "./gato_ANN_dataset_dummy.txt"
    ...
    T = np.asarray(data['Tmed'])
    res = fit_sin(tt, T)
    data['Tsin'] = res["fitfunc"](tt)
    
    #do not use all the data
    data.drop(columns=['THORNTHWAITE','HARGREAVES','BLANEY-CRIDDLE'],inplace=True)
...
def simulate_testset(densesize, seqlength, batchsize, filters,Tmax,Tmin,Tmed,Tsin):
    ...
    if not Tmed:
        data.drop(columns=['Tmed'],inplace=True)
    if not Tsin:
...
def bayesOpt_function(densesize, seqlength, batchsize, filters,Tmax,Tmin,Tmed,Tsin):
...
with tf.device("/gpu:0"): # or cpu
    ...
    GLOBAL_SETTINGS = {
        ...
        'learning_rate': 1e-3,
        'verbose': 1,
        'stopset_start': pd.to_datetime('04092003', format='%d%m%Y'),
        'optset_start': pd.to_datetime('04092007', format='%d%m%Y'),
        'test_start': pd.to_datetime('04092011', format='%d%m%Y'),
        'test_end': pd.to_datetime('29042015', format='%d%m%Y')
    }
    ...
    Tmed = int(round(optimizer.max.get("params").get("Tmed")))
    ...
    scores, TestData, sim1, obs1, inimax, testresults_members_all, sim1_uncertainty = simulate_testset(densesize, seqlength, batchsize, filters,Tmax,Tmin,Tmed,Tsin)
...
pyplot.title("Gato Cave Spring", size=15)
...
pyplot.savefig('Plot_Gato.png', dpi=500)
pyplot.show()
